...
class EditorImagens:
    def __init__(self, root):
        ...
        ttk.Button(frame_botoes, text="Salvar Imagem", command=self.salvar_imagem).grid(row=0, column=1, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Histograma", command=self.exibir_histograma).grid(row=0, column=2, padx=5)
        ttk.Button(frame_botoes, text="Contraste", command=self.alargamento_contraste).grid(row=0, column=3, padx=5)
        ttk.Button(frame_botoes, text="Equalização", command=self.equalizacao_histograma).grid(row=1, column=0, padx=5)
        ttk.Button(frame_botoes, text="Passa-Baixa", command=self.aplicar_passabaixa).grid(row=1, column=1, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Passa-Alta", command=self.aplicar_passaalta).grid(row=1, column=2, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Fourier", command=self.exibir_fourier).grid(row=2, column=1, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Filtro Fourier PB", command=self.fourier_passabaixa).grid(row=1, column=3, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Filtro Fourier PA", command=self.fourier_passaalta).grid(row=2, column=0, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Morfologia", command=self.aplicar_morfologia).grid(row=2, column=2, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Segmentação Otsu", command=self.segmentar_otsu).grid(row=2, column=3, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Descritor de Cor", command=self.descritor_cor).grid(row=3, column=0, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Descritor de Textura", command=self.descritor_textura).grid(row=3, column=1, padx=5, pady=5)
        ttk.Button(frame_botoes, text="Descritor de Forma", command=self.descritor_forma).grid(row=3, column=2, padx=5, pady=5)
